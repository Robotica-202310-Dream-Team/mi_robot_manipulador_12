...
class Robot_Recoger_Ficha(Node):
    ...
    def listener_callback_flag_pick_up(self, msg):
        self.msg1.data = msg.data
        if self.msg1.data == True:
            self.Pick_Up()  
        else:
            pass
		
    def listener_callback_flag_place(self, msg):
        self.msg2.data = msg.data
        if self.msg2.data == True:
            self.Place()
        else:
            pass
...
def main(args=None):
    ...
    robot_recoger_ficha = Robot_Recoger_Ficha()
    rclpy.spin(robot_recoger_ficha)
    robot_recoger_ficha.destroy_node()
    rclpy.shutdown()
